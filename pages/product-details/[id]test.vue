    ...
    <div class="p-4 space-y-6">
      ...
      <div>
        ...
        <div class="space-y-4 border p-4 rounded bg-gray-50">
          <div>
            <label class="block text-sm font-medium mb-1">上記メニューに無い商品をご希望の場合に、ご記入下さい。</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium mb-1">仕上げ方</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium mb-1">商品価格（円）</label>
            <input
              type="number"
...
const newCustomItem = ref<CustomItem>({ name: "", price: 0, quantity: 1, fileName: "" });
const isSubmitting = ref(false);

// -------------------- 商品詳細 & 追加オプション取得 --------------------
onMounted(async () => {
  try {
    product.value = await $fetch<Product>(`/api/product-details/${productId}`);

    const options = await $fetch<OptionItem[]>("/api/print-options", { params: { productId } });
    additionalItems.value = options.map(item => ({ ...item, selected: false, quantity: 0 }));
    printOptions.value = options.map(item => ({ id: item.id, label: item.label, value: item.value, quantity: 1 }));

  } catch (err) {
    console.error("商品または追加オプションの取得に失敗:", err);
  }
...
const sendOrderEmail = async () => {
  ...
  try {
    ...
    const orderDetails = {
      ...
      items: printOptions.value ?? [],
      additionalItems: selectedAdditionalItems.value ?? [],
      customItems: customItems.value ?? [],
      images: favoritesStore.favoriteImages ?? [],
      totalWithTax: totalPriceWithTax.value ?? 0,
    };

    // -------------------- HTML生成関数 --------------------
    const makeListHtml = (arr: any[], renderFn: (item: any, idx?: number) => string) =>
      arr.length > 0 ? `<ul>${arr.map(renderFn).join("")}</ul>` : "";

    const itemsHtml = makeListHtml(orderDetails.items, (item) =>
      `<li>${item.label || "商品名未設定"} × ${item.quantity || 1}：¥${(
        item.price || 0
      ).toLocaleString()}</li>`
    );

    const additionalHtml = makeListHtml(orderDetails.additionalItems, (item) =>
      `<li>${item.label || "追加商品"} × ${item.quantity || 1}：¥${(
        (item.price || 0) * (item.quantity || 1)
      ).toLocaleString()}</li>`
    );

    const customHtml = makeListHtml(orderDetails.customItems, (item) =>
      `<li>${item.name || "カスタム"} × ${item.quantity || 1}：¥${(
        (item.price || 0) * (item.quantity || 1)
      ).toLocaleString()}</li>`
    );

    const imageListHtml = makeListHtml(orderDetails.images, (img, idx) =>
      `<li>画像 ${idx + 1}: <a href="${img.url}" target="_blank">${img.fileName || img.url}</a></li>`
    );
    ...
    const html = `
      ...
      ${orderDetails.comment ? `<p>備考: ${orderDetails.comment}</p>` : ""}
      <p>ご利用ありがとうございました。</p>
    `;
